...
def random_spatial_picai_augmentation(image, label, confg_path='/picai-segmentation/config/config.yaml'):
...
def random_intensity_picai_augmentation(image, confg_path='/picai-segmentation/config/config.yaml'):
...
def random_augment(image, label, confg_path='/picai-segmentation/config/config.yaml'):
    """
    Parameters
    ----------
    image: torch tensor (n, c, d, h, w)
    ...
    for image_file, label_file in zip(image, label):
        ...
        if random() < params['augmentation']['general_spatial_probability']:
            image_file, label_file = random_spatial_picai_augmentation(image_file, label_file, confg_path)

            image_file = image_file.float()
            label_file = label_file.long()

        if random() < params['augmentation']['general_intensity_probability']:
            image_file = random_intensity_picai_augmentation(image_file, confg_path)
...
def patch_cropper(image, label, confg_path='/picai-segmentation/config/config.yaml'):
    """
    Parameters
    ----------
    image: torch tensor (n, c, d, h, w)
    label: torch tensor (n, d, h, w)
    confg_path: str

    Returns
    -------
    transformed_image_list: torch tensor (n, c, d, h, w)
    transformed_label_list: torch tensor (n, d, h, w)
    """
    params = read_config(confg_path)
    image_list = []
    label_list = []
